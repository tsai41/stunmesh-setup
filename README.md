...
stunmesh-go itself needs no manual install: setup downloads the official prebuilt release binary (v1.9.0) from [upstream releases](https://github.com/tjjh89017/stunmesh-go/releases). To build from source instead, follow the [upstream build guide](https://github.com/tjjh89017/stunmesh-go#build) and place the `stunmesh-go` binary in `state/`.
...
Custom tunnel IPs: `IP=<self> PEER_IP=<peer>` (same /24; defaults A=10.66.0.1, B=10.66.0.2). Must mirror each other on the two machines — A's `IP` is B's `PEER_IP` and vice versa. Re-runs keep previously set IPs.
...
Other targets: `make status`, `make logs`, `make next`. (The scripts live in [`scripts/`](scripts/) and can also be run directly, e.g. `./scripts/setup.sh --node A`.)
...
- `state/settings.env` — node identity (tunnel IPs + peer public key), written by setup; not secret
